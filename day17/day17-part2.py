        ...
        if representation[i][j] == "#":
            ...
            for x in range(i-1, i+2):
                for y in range(j-1, j+2):
                    for z in range(-1, 2):
                        for w in range(-1, 2):
                            state[str([i,j,0,0])].append([x,y,z,w])
            state[str([i,j,0,0])].remove([i,j,0,0])
...
while cycle < 6:
    ...
    for each in possible_changes:
        if possible_changes[each] == 3 and not each in state:
            ...
            neighborW = ast.literal_eval(each)[3]
            for x in range(neighborX-1, neighborX+2):
                for y in range(neighborY-1, neighborY+2):
                    for z in range(neighborZ-1,neighborZ+2):
                        for w in range(neighborW-1,neighborW+2):
                            state[each].append([x,y,z,w])
            state[each].remove([neighborX,neighborY,neighborZ,neighborW])
